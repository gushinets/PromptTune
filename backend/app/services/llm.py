...
GOAL_PROMPT_HINTS: dict[tuple[AudienceMode, CanonicalGoal], str] = {
    ("ai", "chatgpt"): (
        "Оптимизируй промпт под ChatGPT: разбей задачу на явные шаги, пропиши "
        "критерии качества результата, укажи ожидаемый формат вывода. Если задача "
        "многоэтапная — попроси модель выполнять шаги последовательно и показывать "
        "промежуточный результат перед финальным."
    ),
    ("ai", "claude"): (
        "Оптимизируй промпт под Claude: добавь чёткий контекст задачи, явно пропиши "
        "ограничения и то, чего делать не нужно. Если задача требует рассуждения — "
        "попроси модель сначала изложить логику, затем дать ответ. Формулируй "
        "требования позитивно: что нужно сделать, а не только чего избегать."
    ),
    ("ai", "perplexity"): (
        "Оптимизируй промпт под Perplexity: сформулируй запрос как исследовательский "
        "вопрос. Явно укажи, что ожидаются ссылки на источники, проверяемые факты и "
        "актуальные данные. Если тема требует сравнения — попроси структурировать "
        "ответ по источникам или точкам зрения."
    ),
    ("ai", "structured"): (
        "Оптимизируй промпт под structured output: явно укажи нужные поля, их названия "
        "и типы данных. Попроси модель вернуть результат строго в заданном формате "
        "(JSON, таблица, список с фиксированными полями). Если формат не указан "
        "пользователем — выбери наиболее подходящий по смыслу задачи и пропиши его явно."
    ),
    ("ai", "deep_research"): (
        "Оптимизируй промпт под глубокое исследование: попроси модель не "
        "ограничиваться очевидными ответами, а рассмотреть несколько точек зрения, "
        "выдвинуть и проверить гипотезы, провести сравнительный анализ. Укажи, что "
        "в финале нужны обоснованные выводы, а не просто перечисление фактов."
    ),
    ("content", "general"): (
        "Оптимизируй промпт под создание контента: явно укажи тон, целевую аудиторию "
        "и платформу публикации, если они следуют из запроса. Результат должен быть "
        "готов к использованию без дополнительной правки."
    ),
    ("content", "seo_article"): (
        "Оптимизируй промпт под SEO-статью: явно укажи целевой поисковый запрос и "
        "намерение читателя (informational / commercial / transactional), только "
        "если они указаны пользователем или прямо следуют из запроса. Попроси "
        "структуру с H1/H2 заголовками, органичным вхождением заданных ключевых слов, "
        "мета-описанием до 160 символов и чётким выводом. Не добавляй неподтверждённые "
        "ключевые слова или поисковые намерения."
    ),
    ("content", "product_description"): (
        "Оптимизируй промпт под товарное описание: сделай явными выгоды для "
        "покупателя, ключевые характеристики товара и призыв к действию, если они "
        "указаны пользователем или прямо следуют из запроса. Закрытие типичных "
        "возражений добавляй только при наличии исходных данных для них. Тон должен "
        "соответствовать платформе и аудитории, если они указаны."
    ),
    ("content", "ad_copy"): (
        "Оптимизируй промпт под рекламный текст: структура — сильный hook, чёткий "
        "оффер и короткий CTA. Элемент доверия (proof) добавляй только если он указан "
        "пользователем или прямо следует из запроса. Попроси вариант, готовый к "
        "размещению, без лишних вступлений и неподтверждённых утверждений."
    ),
    ("content", "email"): (
        "Оптимизируй промпт под email: результат должен включать тему письма "
        "(subject line), если не указано иное. Структура: контекст → суть → чёткий "
        "следующий шаг. Тон — вежливый и дружелюбный для клиентских писем, "
        "нейтральный деловой для деловой переписки, если не задано другое. Письмо "
        "должно быть готово к отправке без правки."
    ),
    ("content", "landing_page"): (
        "Оптимизируй промпт под лендинг: структура — оффер, value proposition и CTA. "
        "Блок social proof добавляй только если он указан пользователем или прямо "
        "следует из запроса. Попроси текст по секциям с явными заголовками. Каждая "
        "секция должна решать одну задачу читателя и вести к целевому действию без "
